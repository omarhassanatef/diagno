...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "sift-config-e2e-"));
});
...
function runCli(args: string[]): Promise<{ stdout: string; code: number | null }> {
  return new Promise((resolve) => {
    const child = spawn(tsxBin, [cliEntry, ...args], {
      ...
      env: { ...process.env, ANTHROPIC_API_KEY: "", HOME: tmpHome, USERPROFILE: tmpHome },
    });
    let stdout = "";
...
describe("sift config (end-to-end)", () => {
  it("round-trips setting and getting the API key through the real CLI", async () => {
    const setResult = await runCli(["config", "set", "api-key", "sk-ant-realkey123"]);
    ...
    const configPath = path.join(tmpHome, ".sift", "config.json");
  ...
  it("a stored api-key enables the AI fallback path (attempted, not skipped)", async () => {
    ...
    // Ambiguous failure -> SIFT will *try* to call the AI provider now that
  ...
  it("without a stored key or env var, the AI fallback is skipped with a helpful message", async () => {
    ...
    expect(result.stdout).toContain("sift config set api-key");
  });

  it("config path reflects the overridden home directory", async () => {
    const result = await runCli(["config", "path"]);
    expect(result.stdout.trim()).toBe(path.join(tmpHome, ".sift", "config.json"));
  });
});
